...
- **Example category chips (Examples panel):** Lecture is primary (curriculum), Function is info-cyan (informational about an analytic f(x)), Demo is warning amber (the Runge example IS the cautionary tale, and amber pre-flags the warning bar that example will produce). Amber's app-wide meaning of "numerical caution" stays intact.
